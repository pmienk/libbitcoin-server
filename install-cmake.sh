...
BITCOIN_SERVER_BRANCH="generated-rewrite-cmake"
...
build_all()
{
    ...
    build_from_github_cmake libbitcoin-node "builds/cmake" "$PARALLEL" false "yes" "${BITCOIN_NODE_OPTIONS[@]}" $CUMULATIVE_FILTERED_ARGS_CMAKE "$@"
    export CPPFLAGS=$SAVE_CPPFLAGS
    local SAVE_CPPFLAGS="$CPPFLAGS"
    export CPPFLAGS="$CPPFLAGS ${BITCOIN_SERVER_FLAGS[@]}"
    if [[ ! ($CI == true) ]]; then
        create_from_github pmienk libbitcoin-server ${BITCOIN_SERVER_BRANCH} "yes"
    build_from_github_cmake libbitcoin-server "builds/cmake" "$PARALLEL" true "yes" "${BITCOIN_SERVER_OPTIONS[@]}" $CUMULATIVE_FILTERED_ARGS_CMAKE "$@"
    else
