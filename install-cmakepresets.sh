...
build_all()
{
    ...
    build_from_github_preset libbitcoin-node "builds/cmake" ${REPO_PRESET[libbitcoin-node]} "$PARALLEL" false "yes" "${BITCOIN_NODE_OPTIONS[@]}" $CUMULATIVE_FILTERED_ARGS_CMAKE "$@"
    export CPPFLAGS=$SAVE_CPPFLAGS
    local SAVE_CPPFLAGS="$CPPFLAGS"
    ...
    if [[ ! ($CI == true) ]]; then
        ...
        build_from_github_preset libbitcoin-server "builds/cmake" ${REPO_PRESET[libbitcoin-server]} "$PARALLEL" true "yes" "${BITCOIN_SERVER_OPTIONS[@]}" $CUMULATIVE_FILTERED_ARGS_CMAKE "$@"
    else
